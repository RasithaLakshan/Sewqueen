...
Telegram: https://t.me/dmass2004
Facebook: https://www.facebook.com/profile.php?id=100070958045517
...
const heroku = new Heroku({
    token: Rasiya.HEROKU.API_KEY
});
const ytdl = require('ytdl-core');
const ffmpeg = require('fluent-ffmpeg');
let baseURI = '/apps/' + Rasiya.HEROKU.APP_NAME;
const Language = require('./language');
const Lang = Language.getString('updater');

// Sql
const SewQueenDB = Rasiya.DATABASE.define('QueenSewWhatsappBot', {
    info: {
...
async function sewQueen () {
    ...
    setInterval(async () => { 
        ...
        var ann_msg = await Pach.news_daily(Rasiya.LANG)
        var ann = await Pach.rnn()
        while (getGMTh == 18 && getGMTm == 00) {
            var ilan = ''
            if (Rasiya.LANG == 'EN') ilan = '| *✨Daily Announcements For Sew Queen✨* |\n\n'
            if (Rasiya.LANG == 'SI') ilan = '| *✨Sew Queen වට්සැප් බොට් සදහා දෛනික නිවේදන✨* |\n\n'
    ...
    setInterval(async () => { 
        ...
        var anomsg = await Pach.news_daily(Rasiya.LANG)
        var aaq = await Pach.rnn()
        while (getGMTq == 08 && getGMTs == 00) {
            var srmg = ''
            if (Rasiya.LANG == 'EN') srmg = '| *✨Daily Announcements For Sew Queen✨* |\n\n'
            if (Rasiya.LANG == 'SI') srmg = '| *✨Sew Queen වට්සැප් බොට් සදහා දෛනික නිවේදන✨* |\n\n'
    ...
    asynchronous_ch()
    setInterval(async () => { 
        if (Rasiya.AUTOBIO == 'true') {
            var timezone_bio = await Pach.timezone(CdSew.user.jid)
            var date_bio = await Pach.datebio(Rasiya.LANG)
    ...
    var logger_levels = ''
    if (Rasiya.DEBUG == 'true') {
        logger_levels = 'all'
    } else if (Rasiya.DEBUG == 'false') {
        logger_levels = 'off'
    } else if (Rasiya.DEBUG == 'trace') {
        logger_levels = 'trace'
    } else if (Rasiya.DEBUG == 'fatal') {
        logger_levels = 'fatal'
    } else if (Rasiya.DEBUG == 'warn') {
        logger_levels = 'warn'
    } else if (Rasiya.DEBUG == 'error') {
        logger_levels = 'error'
    } else if (Rasiya.debug == 'info') {
        logger_levels = 'info'
    } else {
        logger_levels = 'warn'
    }
    ...
    if (StrSes_Db.length < 1) {
        nodb = true;
        CdSew.loadAuthInfo(Session.deCrypt(Rasiya.SESSION)); 
    } else {
...
${chalk.white.bold('Version:')} ${chalk.red.bold(Rasiya.VERSION)}
...
${chalk.blue.italic('🇱🇰 Try To Login WhatsApp... Please Wait...')}`);
    ...
    CdSew.on('credentials-updated', async () => {
        ...
        let rssewqueengg = Rasiya.WORKTYPE == 'public' ? ' Public' : ' Private'
        ...
        if (CdSew.user.jid == one || CdSew.user.jid == two || CdSew.user.jid == three || CdSew.user.jid == four || CdSew.user.jid == five || CdSew.user.jid == six || CdSew.user.jid == seven || CdSew.user.jid == eight) {
            ...
            var sew_start = await Pach.work_type(Rasiya.WORKTYPE, Rasiya.LANG, Rasiya.FULLSEW)
            var sew_img = await Pach.img_type(Rasiya.WORKTYPE, Rasiya.FULLSEW)
        ...
        await git.fetch();
        var commits = await git.log([Rasiya.BRANCH + '..origin/' + Rasiya.BRANCH]);
        if (commits.total === 0) {
            ...
            var up_ch = await Pach.update_rs(Rasiya.LANG)
    ...
    CdSew.on('message-new', async msg => {
        ...
        // ==================== Greetings ====================
        if (Rasiya.GIFORPP == 'pp' || Rasiya.GIFORPP == 'Pp' || Rasiya.GIFORPP == 'PP' || Rasiya.GIFORPP == 'pP' ) {
    ...
    else if (Rasiya.GIFORPP == 'gif' || Rasiya.GIFORPP == 'Gif' || Rasiya.GIFORPP == 'GIF' || Rasiya.GIFORPP == 'GIf' ) {
    if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            // welcome
            var gb = await getMessage(msg.key.remoteJid, 'goodbye');
            if (gb !== false) {
                var sewqueenimage = await axios.get(Rasiya.WLP, { responseType: 'arraybuffer' })
            ...
            var gb = await getMessage(msg.key.remoteJid);
            if (gb !== false) {
            var sewqueenimage = await axios.get(Rasiya.GDB, { responseType: 'arraybuffer' })
            await CdSew.sendMessage(msg.key.remoteJid, Buffer.from(sewqueenimage.data), MessageType.video, {mimetype: Mimetype.gif, caption: gb.message +'\n\n                 ᴘᴏᴡᴇʀᴅ ʙʏ ꜱᴇᴡ ǫᴜᴇᴇɴ'});
            }
            return;
        }
     }
     else if (Rasiya.GIFORPP == 'img' || Rasiya.GIFORPP == 'Img' || Rasiya.GIFORPP == 'IMG' || Rasiya.GIFORPP == 'image' ) {
    if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            // welcome
            var gb = await getMessage(msg.key.remoteJid, 'goodbye');
            if (gb !== false) {
        ...
        // ==================== Blocked Chats ====================
        if (Rasiya.BLOCKCHAT !== false) {     
            var abc = Rasiya.BLOCKCHAT.split(',');                            
            if(msg.key.remoteJid.includes('-') ? abc.includes(msg.key.remoteJid.split('@')[0]) : abc.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.SUPPORT == '94785435462-1627812354') {     
            var sup = Rasiya.SUPPORT.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sup.includes(msg.key.remoteJid.split('@')[0]) : sup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.SUPPORT2 == '94785435462-1628835469') {     
            var tsup = Rasiya.SUPPORT2.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.SUPPORT3 == '94785435462-1628835633') {     
            var nsup = Rasiya.SUPPORT3.split(',');                            
            if(msg.key.remoteJid.includes('-') ? nsup.includes(msg.key.remoteJid.split('@')[0]) : nsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.RRASEW == '94785435462-1621751150') {     
            var sewrm = Rasiya.RRASEW.split(',');                            
        ...
        if (Rasiya.RRBSEW == '94785435462-1625490851') {     
            var sewrm = Rasiya.RRBSEW.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sewrm.includes(msg.key.remoteJid.split('@')[0]) : sewrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.RRCSEW == '94785435462-1618586156') {     
            var sewrm = Rasiya.RRCSEW.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sewrm.includes(msg.key.remoteJid.split('@')[0]) : sewrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.RRDSEW == '94776785357-1626432386') {     
            var sewrm = Rasiya.RRDSEW.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sewrm.includes(msg.key.remoteJid.split('@')[0]) : sewrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.RRESEW == '94776785357-1626521320') {     
            var sewrm = Rasiya.RRESEW.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sewrm.includes(msg.key.remoteJid.split('@')[0]) : sewrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.RRFSEW == '94785435462-1618915104') {     
            var sewrm = Rasiya.RRFSEW.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sewrm.includes(msg.key.remoteJid.split('@')[0]) : sewrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.RRRRA == '393475528094-1415817281') {     
            var sewrm = Rasiya.RRRRA.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sewrm.includes(msg.key.remoteJid.split('@')[0]) : sewrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.RRRRB == '96176912958-1458298055') {     
            var sewrm = Rasiya.RRRRB.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sewrm.includes(msg.key.remoteJid.split('@')[0]) : sewrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Rasiya.RRRRC == '393472769604-1446476993') {     
            var sewrm = Rasiya.RRRRC.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sewrm.includes(msg.key.remoteJid.split('@')[0]) : sewrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        // ==================== End Blocked Chats ====================
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if ((Rasiya.SUDO !== false && msg.key.fromMe === false && command.fromMe === true &&
                        (msg.participant && Rasiya.SUDO.includes(',') ? Rasiya.SUDO.split(',').includes(msg.participant.split('@')[0]) : msg.participant.split('@')[0] == Rasiya.SUDO || Rasiya.SUDO.includes(',') ? Rasiya.SUDO.split(',').includes(msg.key.remoteJid.split('@')[0]) : msg.key.remoteJid.split('@')[0] == Rasiya.SUDO)
                    ...
                    // ==================== Message Catcher ====================
                    if (sendMsg) {
                        if (Rasiya.SEND_READ && command.on === undefined) {
                        ...
                        try {
                            ...
                            if (Rasiya.NOLOG == 'true') return;
                            var error_report = await Pach.error(Rasiya.LANG)
